...
public interface AssistanceAgentDashboardRepository extends AbstractRepository {
	...
	// Ratio de claims aceptadas
	@Query("SELECT 1.0 * COUNT(c) / (SELECT COUNT(c1) FROM Claim c1) FROM Claim c WHERE c.indicator = :indicator")
	Double ratioOfClaimsStoredSuccessfully(IndicatorType indicator);

	// Ratio de claims rechazadas
	@Query("SELECT 1.0 * COUNT(c) / (SELECT COUNT(c1) FROM Claim c1) FROM Claim c WHERE c.indicator = :indicator")
	Double ratioOfClaimsRejected(IndicatorType indicator);
	...
	List<Integer> topThreeMonthsHighestNumberOfClaims(Pageable pageable); // usar Pageable.ofSize(3)
